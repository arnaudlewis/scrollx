...
function filmDuration(convertedScenes, computed) { //in PX
...
function compute(scenes, animationNode) {
  ...
  const duration = filmDuration(convertedScenes, computed)
...
function setCssProperties(node, properties) {
  node.style.transform = `translate3d(${properties[Property.TranslateX].value}${properties[Property.TranslateX].unit}, ${properties[Property.TranslateY].value}${properties[Property.TranslateY].unit}, 0) scale(${properties[Property.Scale].value})`
